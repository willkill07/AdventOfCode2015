...
isNice (const std::string& str) {
	return std::regex_search (str, threeVowels) &&
		std::regex_search (str, doubleLetter) &&
		!std::regex_search (str, bad);
}

bool
isNice2 (const std::string& str) {
	return std::regex_search (str, pair) &&
		std::regex_search (str, post);
}
