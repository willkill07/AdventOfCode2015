...
template <Day> double timeSolve (bool, bool);

double run (Day, bool, time_options_t, std::ostream &);
...
template <Day DAY> double timeSolve (bool part2, bool time, std::ostream & os) {
  double resTime { 0.0 };
...
#define RUN_DAY(X) case Day ## X : return timeSolve <Day ## X> (part2, time, os);
double run (Day day, bool part2, time_options_t time, std::ostream & os) {
  switch (day) {
